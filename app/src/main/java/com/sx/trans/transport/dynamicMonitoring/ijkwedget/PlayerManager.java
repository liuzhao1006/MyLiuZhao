...
public class PlayerManager implements OnClickListener, BaseInfo {
    /**
...
//    Httpost httpost;
    ...
    public PlayerManager(final Activity activity) {
        ...
        // currentPosition = (int)
        // FormatUtil.forLong(activity.getIntent().getStringExtra("Playtime"));
        screenWidthPixels = activity.getResources().getDisplayMetrics().widthPixels;

        videoView = (IjkVideoView) activity.findViewById(R.id.video_view);
        progressBar = (ProgressBar) activity.findViewById(R.id.progbar);
        downTv = (TextView) activity.findViewById(R.id.down_tv);
        img_btn = (ImageButton) activity.findViewById(R.id.img_btn);
            ...
            public boolean onInfo(IMediaPlayer mp, int what, int extra) {
                switch (what) {
                    ...
                    case IMediaPlayer.MEDIA_INFO_NETWORK_BANDWIDTH:
                        // 显示下载速度
                        // Toast.show("download rate:" + extra);T

                        downTv.setVisibility(View.VISIBLE);
                        downTv.setText("" + extra + "kb/s" + "  ");
                        break;
...
//		DebugLog.d("brightness:" + brightness + ",percent:" + percent);
    ...
    class Query {
        ...
        public int dip2pixel(Context context, float n) {
            int value = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, n,
    ...
    public class PlayerGestureListener extends GestureDetector.SimpleOnGestureListener {
        ...
        public boolean onDown(MotionEvent e) {
            firstTouch = true;
            activity.findViewById(R.id.titile_tv).setVisibility(View.VISIBLE);
            activity.findViewById(R.id.tips).setVisibility(View.VISIBLE);
...
//	public PlayerManager onPrepared(OnPreparedListener onPreparedListener) {
...
//		void onPrepared(MediaPlayer mediaPlayer);
    ...
    private void setImageViewSelect(boolean flag) {
        img_btn.setSelected(flag);
        if (flag) {
            onPause();
        } else {
            onResume();
...
//    BreakpointControl breakpointControl;
    ...
    private void GetVideoTimer() {//  获取播放时间
//        breakpointControl = new BreakpointControl(PlayerManager.this, activity);
//        breakpointControl.StuId = (String) SpUtils.get(activity, "StuId", "");
//        breakpointControl.VideoId = activity.getIntent().getStringExtra("VidoId");
//        breakpointControl.StudyPlanId = activity.getIntent().getStringExtra("StudyPlanId");
//        breakpointControl.CompanyId = (String) SpUtils.get(activity, "Company", "");
//        breakpointControl.doRequest();
    }

    public void setIsFinish(String isFinish) {  //是否播放完毕
        this.IsFinish = isFinish;
    }

//    InPutBreakPointControl inPutBreakPointControl;

    public void setVideoTime() {//  插入视频断点
//        inPutBreakPointControl = new InPutBreakPointControl(PlayerManager.this, activity);
//        inPutBreakPointControl.StuId = (String) SpUtils.get(activity, "StuId", "");
//        inPutBreakPointControl.VideoId = activity.getIntent().getStringExtra("VidoId");
//        inPutBreakPointControl.StudyPlanId = activity.getIntent().getStringExtra("StudyPlanId");
//        inPutBreakPointControl.CompanyId = (String) SpUtils.get(activity, "Company", "");
//        inPutBreakPointControl.NowTime = videoView.getCurrentPosition() / 1000 + "";
//        inPutBreakPointControl.IsFinish = IsFinish;
//        inPutBreakPointControl.Id = ID;
//        inPutBreakPointControl.doRequest();
    }
...
//    @Override
    public void doRequestFall(String info, BaseBean mBaseBean) {
//        if (mBaseBean.mControlCode == BaseBean.ControlCode.BreakpointControl) {
//            if (mBaseBean.returnCode == 0) {
//                Message mes = handler1.obtainMessage();
//                currentPosition = 0;
//                mes.obj = currentPosition;
//                mes.what = 0;
...
//    @Override
    public void doRequestSuccess(String info, BaseBean mBaseBean) {
//        if (mBaseBean.mControlCode == BaseBean.ControlCode.BreakpointControl) {
//            GetBreakPointBean ban = (GetBreakPointBean) mBaseBean;
//            if (ban.returnDate.get(0).Id != null) {
//                ID = ban.returnDate.get(0).Id;
//            }
//
//            if (ban.returnDate.get(0).NowTime != null) {
//                currentPosition = Integer.valueOf(ban.returnDate.get(0).NowTime) * 1000;
//            } else {
//                currentPosition = 0;
//            }
//
...
//        }
//
//        if (mBaseBean.mControlCode == BaseBean.ControlCode.InPutBreakPointControl) {
//            if (IsFinish.equals("1")) {
//                Toast.makeText(activity, "该视频已结束！请观看其他视频", Toast.LENGTH_LONG);
//                Dialog("视频学时更新成功");
//            }
//        }
    }

    private void Dialog(String str) {//   记录学时成功时弹框提示用户
        LayoutInflater inflater = LayoutInflater.from(activity);
//        RelativeLayout layout = (RelativeLayout) inflater.inflate(R.layout.dialog_xml, null);
...
//        });
    ...
    private Handler handler1 = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case 0:
//                    int postin = (int) msg.obj;
////                    img_btn.setSelected(false);
//                    // if (status == STATUS_PLAYING) {
//                    if (isLive) {
//                        videoView.seekTo(postin);
//                        videoView.start();
//                    } else {
//                        if (postin > 0) {
//                            videoView.seekTo(postin);
